...
void MainBox::showHistory(){
    ...
    for(int i=0;i<9;i++){
        ...
        //kludge to fix spacing bugs, duh
            ...
            tempPixmap.fill(QColor("transparent"));
            histWind.chart[i]->setPixmap(tempPixmap);
            histWind.title[i]->setText("");
            histWind.chart[i]->setFixedSize(211,116);
        }
...
void MainBox::showEconomy(){
    econWind.setModal(true);
    econWind.setParent(this,Qt::Dialog);
    econWind.setWindowTitle(resrc.strg[curSelection+154]+" - economy");

    int x=core.year-1982;
    int tmpx, tmpy, val[2], plot[9], minimum=INT_MAX, maximum=INT_MIN;
    QPixmap *square;
    QVector<QPoint> lineChart;

    QPen lineChartPen;
    lineChartPen.setWidth(1);
    QBrush barFill;
    barFill.setStyle(Qt::SolidPattern);

    //Do the GDP by sectors table
    for(int i=0;i<5;i++){
        square=new QPixmap(75,15);
        square->fill(QColor("transparent"));
        QPainter squarePainter(square);
        squarePainter.setPen(lineChartPen);

        squarePainter.drawText(QPoint(20,11),QString(resrc.strg[871+i]));
        if(i==1) barFill.setColor(QColor(255,255,0));
        else if(i==2) barFill.setColor(QColor(255,127,0));
        else if(i==3) barFill.setColor(QColor(255,0,0));
        else if(i==4) barFill.setColor(QColor(0,127,0));
        squarePainter.setBrush(barFill);
        if(i>0) squarePainter.drawRect(4,2,10,10);
        econWind.gdpCol[i]->setPixmap(*square);
        econWind.title[i]->setText("<b>"+resrc.strg[879+i]+"</b>");
    }

    for(int i=5;i<20;i++){
        if(!(i%5)) econWind.gdpCol[i]->setText("<p align=\"right\">"+resrc.strg[875+i/5]+"</p>");
        else if(i>=6&&i<=8)
            econWind.gdpCol[i]->setText("<p align=\"right\">"+captionCreate(33,0,core.state[curSelection].gdpSpending[i-6])+"</p>");
        else if(i>=11&&i<=13)
            econWind.gdpCol[i]->setText("<p align=\"right\">"+captionCreate(33,1,core.state[curSelection].gdpSpending[i-11]/(core.state[curSelection].population/1000))+"</p>");
        else if(i>=16&&i<=18){
            QString tempText=captionCreate(33,2,(core.state[curSelection].gdpFrac[i-16]*100000/255));
            tempText.resize(tempText.size()-7);
            if(core.year>1983){
                if(core.state[curSelection].gdpFrac[i-16]>core.history[x-1][curSelection][i+11])
                    tempText.append(" \u25b2");
                else if(core.state[curSelection].gdpFrac[i-16]<core.history[x-1][curSelection][i+11])
                    tempText.append(" \u25bc");
                else tempText.append(" \u2501");
            }
            econWind.gdpCol[i]->setText("<p align=\"right\">"+tempText+"</p>");
        }
        else if(i==9)
            econWind.gdpCol[i]->setText("<p align=\"right\">"+captionCreate(33,0,core.state[curSelection].gdp)+"</p>");
        else if(i==14)
            econWind.gdpCol[i]->setText("<p align=\"right\">"+captionCreate(33,1,core.state[curSelection].gdp/(core.state[curSelection].population/1000))+"</p>");
        else if(i==19)
            econWind.gdpCol[i]->setText("<p align=\"right\">100.000%</p>");
    }

    //Do the charts
    for(int i=0;i<4;i++){
        QPixmap tempPixmap(230,116);
        tempPixmap.fill(QColor("transparent"));
        QPainter painter(&tempPixmap);
        painter.drawLine(QPoint(5,0),QPoint(5,100));    //y-axis
        painter.drawLine(QPoint(205,0),QPoint(205,100));    //second y-axis
        painter.drawLine(QPoint(5,100),QPoint(205,100));    //x-axis
        for(int j=0;j<6;j++){
            painter.drawLine(QPoint(3,100-(j+1)*20),QPoint(7,100-(j+1)*20));    //y-axis scale
            painter.drawLine(QPoint(202,100-(j+1)*20),QPoint(208,100-(j+1)*20));    //second y-axis scale
        }
        for(int j=0;j<8;j++){
            if(j<7) painter.drawLine(QPoint(5+(j+1)*25,98),QPoint(5+(j+1)*25,102));    //x-axis scale
            painter.drawText(QPoint(10+(j)*25,114),QString("'")+QString::number(j+82));
        }

        //First chart, GDP by sectors historically
        if(i==0) {
            for(int j=0;j<=x;j++){
                tmpx=100;
                for(int y=0;y<3;y++){
                    if(y==0) barFill.setColor(QColor(255,255,0));
                    else if(y==1) barFill.setColor(QColor(255,127,0));
                    else barFill.setColor(QColor(255,0,0));
                    painter.setBrush(barFill);
                    painter.drawRect(10+(j)*25,tmpx,15,-core.history[j][curSelection][27+y]/2.55);

                    tmpx-=(core.history[j][curSelection][27+y]/2.55)-1;
                }
            }
        }
        //Rest of charts + data for comparison (and scaling - 1982 vs current)
        else if(i>=1){
            painter.setBrush(QColor(0,127,0));
            minimum=INT_MAX, maximum=INT_MIN;
            for(int j=0;j<=x;j++){
                if(i==1) plot[j]=core.history[j][curSelection][30];
                else if(i==2) plot[j]=core.history[j][curSelection][23];
                else if(i==3) plot[j]=core.history[j][curSelection][23]/(core.history[j][curSelection][30]/1000);
            ...
            lineChartPen.setColor(Qt::black);
            lineChartPen.setWidth(1);
            painter.setPen(lineChartPen);
            for(int j=0;j<=x;j++){
                lineChart.append(QPoint(j*25+5,100-plot[j]));
                painter.drawEllipse(QPoint(j*25+5,100-plot[j]),3,3);
            }
            //then draw the line
            lineChartPen.setColor(QColor(0,127,0));
            lineChartPen.setWidth(2);
            painter.setPen(lineChartPen);
            for(int j=0;j+1<lineChart.size();j++)
                painter.drawLine(lineChart[j],lineChart[j+1]);
            lineChart.clear();

            //finally, the data for tables
            econWind.data[0+4*(i-1)]->setText("<b>1982</b>");
            econWind.data[1+4*(i-1)]->setText("<b>Current</b>");
            if(i==1) {
                tmpx=37;
                val[0]=core.history[0][curSelection][30];
                val[1]=core.state[curSelection].population;

            }
            else if(i==2) {
                tmpx=33, tmpy=0;
                val[0]=core.history[0][curSelection][23];
                val[1]=core.state[curSelection].gdp;
            }
            else if(i==3) {
                tmpx=33, tmpy=1;
                val[0]=core.history[0][curSelection][23]/(core.history[0][curSelection][30]/1000);
                val[1]=core.state[curSelection].gdp/(core.state[curSelection].population/1000);
            }
            econWind.data[2+4*(i-1)]->setText(captionCreate(tmpx,tmpy,val[0]));
            econWind.data[3+4*(i-1)]->setText(captionCreate(tmpx,tmpy,val[1]));
        }
        econWind.chart[i]->setPixmap(tempPixmap);
    }
    //also, add some statistical data for comparison
    econWind.show();
}
