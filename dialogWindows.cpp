...
Economy::Economy(QWidget *parent):QDialog(parent){
    mainLayout=new QBoxLayout(QBoxLayout::TopToBottom);
    for(int i=0;i<2;i++) rowLayout[i]=new QBoxLayout(QBoxLayout::LeftToRight);
    grandTableLayout=new QBoxLayout(QBoxLayout::LeftToRight);
    for(int i=0;i<5;i++){
        chartLayout[i]=new QBoxLayout(QBoxLayout::TopToBottom);
        title[i]=new QLabel("Title #"+QString::number(i));
        chartLayout[i]->addWidget(title[i]);
    }
    ...
    for(int i=0;i<6;i++){
        dataColLayout[i]=new QBoxLayout(QBoxLayout::TopToBottom);
        if(i<5) grandTableLayout->addLayout(dataColLayout[i]);
    }
    for(int i=0;i<20;i++){
        gdpCol[i]=new QLabel("Text #"+QString::number(i));
        dataColLayout[i/5]->addWidget(gdpCol[i]);
    }
    for(int i=0;i<3;i++){
        tableLayout[i]=new QBoxLayout(QBoxLayout::LeftToRight);
        chartLayout[i+2]->addLayout(tableLayout[i]);
    }
    for(int i=0;i<4;i++){
        chart[i]=new QLabel;
        chart[i]->setFixedSize(230,116);
        chartLayout[i+1]->addWidget(chart[i]);
    }
    for(int i=0;i<6;i++){
        lowChartsColumnLayout[i]=new QBoxLayout(QBoxLayout::TopToBottom);
        tableLayout[i/2]->addLayout(lowChartsColumnLayout[i]);
    }
    for(int i=0;i<12;i++){
        data[i]=new QLabel("Data #"+QString::number(i));
        lowChartsColumnLayout[i/2]->addWidget(data[i]);
    }

    setWindowTitle("Economy");
    setFixedSize(700,440);
    setLayout(mainLayout);
